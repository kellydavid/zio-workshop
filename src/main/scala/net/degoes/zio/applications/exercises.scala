...
object hangman extends App {
  ...
  def gameLoop(state: State): ZIO[Console, IOException, State] =
    for {
      ...
      cont      <- if(state.playerWon)
                    putStrLn(s"Congratulation, ${state.name}, you won") *> ZIO.succeed(false)
                    else if (state.playerLost)
                    putStrLn(s"Sorry, ${state.name}, you lost.") *> ZIO.succeed(false)
                    else putStrLn("keep going") *> ZIO.succeed(true)
      state     <- if (cont) gameLoop(state) else ZIO.succeed(state)
  ...
  def getChoice: ZIO[Console, IOException, Char] =
    ...
    console.getStrLn.map(_.toLowerCase.trim.toList).flatMap{
      ...
      case _ =>
          console.putStrLn("you did not enter a single letter") *>
          getChoice
    }

  def getName: ZIO[Console, IOException, String] =
    console.putStrLn("Please enter your name") *> console.getStrLn

  def chooseWord: ZIO[Random, Nothing, String] =
    random.nextInt(Dictionary.length).map(Dictionary.lift(_).getOrElse("defect"))
